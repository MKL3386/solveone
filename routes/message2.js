    ...
    if (typeof MMS_FILE_URL == "string") {
      if (MMS_FILE_URL.length > 4) {
        ...
        if(extension_miss == 'em'){
          ...
          //   headerUrl = `${res.request.uri['href']}`;
          ...
          //     // console.log('URL에 파일 확장자가 존재하지 않음. 헤더 확장자 붙임.');
          //     // 2-A. 확장자를 붙여서 패턴대로 저장
          //     filename = getRandomString(5) + new Date().getTime() + '_' + fileNameUrl + '.' + fileType;

          //     console.log('result :: filename => ', filename);
              
          //     request(requestOptions).pipe(fs.createWriteStream(`../file/upload/${filename}`));

          //     // mms_file_url = path.join(__dirname, '..', '..', 'file', 'upload/', filename);
          //     // console.log('mms_file_url =>', mms_file_url);

          //   }else{

          //     // console.log('URL에 파일 확장자가 존재함.');
          //     // 2-B. 별도 작업 없이 패턴대로 저장
          //     filename = getRandomString(5) + new Date().getTime() + '_' + fileNameUrl;
        ...
        // console.log('header_miss => ', header_miss);
        // console.log('extension_miss => ', extension_miss);
